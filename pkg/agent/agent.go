...
func (i *Initializer) configureGatewayInterface(gatewayIface *interfacestore.InterfaceConfig) error {
	...
	// Loop in all local Subnets on the Node, parse the first address in each Subnet and configure on the host gateway.
	for _, localSubnet := range i.nodeConfig.PodCIDRs {
...
// initNodeLocalConfig retrieves node's subnet CIDR from node.spec.PodCIDRs, which is used for IPAM and setup
...
func (i *Initializer) initNodeLocalConfig() error {
	...
	if node.Spec.PodCIDRs != nil {
		for _, podCIDR := range node.Spec.PodCIDRs {
			...
			i.nodeConfig.PodCIDRs = append(i.nodeConfig.PodCIDRs, localSubnet)
		}
		return nil
	}
	...
	i.nodeConfig.PodCIDRs = []*net.IPNet{localSubnet}
	return nil
}
