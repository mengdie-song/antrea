...
func TestLoadNetConfig(t *testing.T) {
	...
	assert.Equal(
		netCfg.IPAM.Subnet, testNodeConfig.PodCIDRs[0].String(),
...
func init() {
	...
	testNodeConfig = &config.NodeConfig{Name: nodeName, PodCIDRs: []*net.IPNet{nodePodCIDR}, GatewayConfig: gateway}
}
