	...
	for _, podIP := range podIPs {
		...
		flows = append(flows,
			expectTableFlows{
				uint8(10),
				[]*ofTestUtils.ExpectFlow{
					{fmt.Sprintf("priority=200,%s,in_port=%d,dl_src=%s,%s=%s", ipProto, podOFPort, podMAC.String(), nwSrcField, podIP.String()),
						"goto_table:29"},
				},
			},
			expectTableFlows{
				...
				[]*ofTestUtils.ExpectFlow{
					{
						fmt.Sprintf("priority=200,%s,reg0=0x80000/0x80000,%s=%s", ipProto, nwDstField, podIP.String()),
	...
	for _, gwIP := range gwIPs {
		...
		flows = append(flows,
			expectTableFlows{
				...
				[]*ofTestUtils.ExpectFlow{
					{
						fmt.Sprintf("priority=200,%s,dl_dst=%s,nw_dst=%s", ipProtoStr, vMAC.String(), gwIP.String()),
			...
			expectTableFlows{
				...
				flows: []*ofTestUtils.ExpectFlow{
					{
						fmt.Sprintf("priority=210,%s,nw_src=%s", ipProtoStr, gwIP.String()),
						"goto_table:105"},
				},
			},
		)
...
func prepareNodeFlows(tunnelPort uint32, peerSubnet net.IPNet, peerGwIP, peerNodeIP net.IP, vMAC, localGwMAC net.HardwareAddr) []expectTableFlows {
	...
	expFlows = append(expFlows, expectTableFlows{
		...
		[]*ofTestUtils.ExpectFlow{
			{
				fmt.Sprintf("priority=200,%s,%s=%s", ipProtoStr, nwDstFieldName, peerSubnet.String()),
